...
def fast_ptrace_qubit(c_evecs, qdim, trunc_list):
    """
    ...
    term1 = 'nq'  + str(letters[i] for i in range(len(trunc_list)))
    term2 = ',np' + str(letters[i] for i in range(len(trunc_list)))
    res   = '->npq'
    
    shape_tuple = tuple([qdim].append(trunc_list))
    # Find number of eigenstates
    num_eigs = c_evecs.shape[0] # shape (num_eigs, qdim*rdim)
    # Reshape to (num_eigs, qdim, rdim)
    psi = c_evecs.reshape(num_eigs, shape_tuple)
    # Compute rho = psi @ psi^† over resonator index
    rho = np.einsum(term1 + term2 + res, psi, psi.conj()) # shape (num_eigs, qdim, qdim)
...
def branch_analysis(pkg: ObjPackage, update_flux):
    ...
    for t_idx in range(np.asarray(map_list).shape[1]):
        ...
        if cdim:
            ...
            label = f"q{int(np.round(n_q_branch[0]))}_c{int(np.round(n_c_expect[idx_list][0]))}"  # Combine qubit and chain indices
        else:
...
def comp_ss_dat(res_list, flux_arr, save=False):
    """
    Given list of all results and the flux array used in sweep,
    repackage all the data to be appropriately plotted in computational
    subspace.
    
    Inputs:
      - res_list   : matrix of bare state/dressed state overlaps          (list)
      - flux_arr   : set of indices which have been used in previous maps (numpy array)

    Returns:
      - dat0       : ground state data dictionary  (dict)
      - dat1       : excited state data dictionary (dict)
    """
